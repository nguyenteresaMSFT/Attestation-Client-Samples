...
vector<uint8_t> send_to_pluton_att_service(const uint8_t* data, size_t size)
{
    try
    {
        //
...
void attest_pluton(const char* session_type, const void* params, const string& file_name)
{
    cout << "Starting attestation..." << endl;
    ...
    while (!complete)
    {
        ...
        if (send_to_server_size > 0)
        {
            received_from_server = send_to_pluton_att_service(send_to_server.get(), send_to_server_size);
        }
    }
